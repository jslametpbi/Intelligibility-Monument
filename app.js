const STORAGE_KEY = 'intelligibilityMonumentPro.v2';
const BUILD_VERSION = '2026.06.04-spot-qr-direct-final';
...
const LEGACY_STATE_KEYS = [
  'intelligibilityMonumentPro.v5.final',
  'intelligibilityMonumentPro.v4.final',
  'intelligibilityMonumentPro.v3.final',
  'intelligibilityMonumentPro.v3',
  'intelligibilityMonumentPro.v2',
  'intelligibilityMonumentPro.v1',
  'intelligibilityMonument.smooth.v7',
  'intelligibilityMonument.final.fixed.v6',
  'imon.clean.release.20260604'
];
...
function migrateLegacyState() {
  ...
  for (const key of LEGACY_STATE_KEYS) {
    ...
    try {
      ...
      if (parsed && (parsed.voices || parsed.identity || Array.isArray(parsed.voices))) {
...
function selectModelHotspotDirect(id) {
  const h = getHotspot(id);
  activeHotspotId = h.id;
  updateModelSelection();
  renderModelSpotPanel();
  renderHotspotPanel();
  toast(`Selected: ${h.title}`);
}
window.selectModelHotspotDirect = selectModelHotspotDirect;
...
function render360Model() {
  ...
  const hotspotMarkup = HOTSPOTS.map(h => {
    ...
    return `
      <button class="model-hotspot ${spec.align === 'right' ? 'alt' : ''} ${h.id === activeHotspotId ? 'active' : ''}" style="left:${spec.x}%;top:${spec.y}%" data-model-hotspot="${h.id}" onclick="window.selectModelHotspotDirect('${h.id}'); return false;" onpointerdown="event.stopPropagation();" type="button" aria-label="Open ${esc(h.title)} hotspot in 360 view">
        <span class="dot" aria-hidden="true">＋</span><span class="label">${esc(h.title)}</span>
      </button>`;
  }).join('');
  stage.innerHTML = `
  ...
  $all('[data-model-hotspot]', stage).forEach(btn => {
    btn.addEventListener('click', event => {
      ...
      selectModelHotspotDirect(btn.dataset.modelHotspot);
    });
...
function toggleModelAuto() {
  ...
  modelAutoTimer = setInterval(() => setModelRotation(modelRotation + 1), 60);
...
function init360Events() {
  ...
  viewport.addEventListener('pointermove', e => {
    ...
    setModelRotation(modelDragStart.rotation + delta * .45);
  });
...
function selectHotspot(id, updateHash = false) {
  const h = getHotspot(id);
  activeHotspotId = h.id;
  if (updateHash) history.replaceState(null, '', `#hotspot=${encodeURIComponent(h.id)}`);
  renderHotspots();
  updateModelSelection();
  renderModelSpotPanel();
  renderHotspotPanel();
...
function renderModelSpotPanel() {
  ...
  const link = `${deepLink('hotspot', h.id)}&selected=${encodeURIComponent(h.id)}`;
  panel.dataset.selectedSpot = h.id;
  panel.innerHTML = `
    <p class="eyebrow">Selected spot QR</p>
    <h3>${esc(h.title)}</h3>
    <img class="qr-side-large" src="${qrImageUrl(link)}&cache=${encodeURIComponent(h.id)}" alt="QR for ${esc(h.title)}" />
    <p><strong>QR target:</strong> ${esc(h.title)}</p>
  ...
  showModal(`
    ...
    <div class="modal-grid qr-modal-grid">
      <div class="qr-card" style="margin:0">
        ...
        <img src="${qrImageUrl(target.url)}" alt="QR for ${esc(target.title)}" style="width:320px;height:320px;margin:18px auto" />
...
function initEvents() {
  document.addEventListener('click', e => {
    const btn = e.target.closest?.('[data-model-hotspot]');
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    selectModelHotspotDirect(btn.dataset.modelHotspot);
  ...
  $all('.main-nav a').forEach(a => a.addEventListener('click', e => {
