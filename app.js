...
const BUILD_VERSION = '2026.06.04-hotspot-smooth-fix';
...
function render360Model() {
  ...
  $all('[data-model-hotspot]', stage).forEach(btn => {
    const selectFrom360 = event => {
      event.preventDefault();
      event.stopPropagation();
      selectHotspot(btn.dataset.modelHotspot, false);
      renderModelSpotPanel();
    };
    btn.addEventListener('click', selectFrom360);
    btn.addEventListener('pointerup', selectFrom360);
  });
...
function toggleModelAuto() {
  ...
  modelAutoTimer = setInterval(() => setModelRotation(modelRotation + 1), 45);
...
function init360Events() {
  ...
  viewport.addEventListener('pointermove', e => {
    ...
    setModelRotation(modelDragStart.rotation + delta * .5);
  });
...
function selectHotspot(id, updateHash = false) {
  ...
  renderModelSpotPanel();
  renderHotspotPanel();
  toast(`Selected: ${getHotspot(id).title}`);
}
...
function renderModelSpotPanel() {
  ...
  panel.innerHTML = `
    ...
    <h3>${esc(h.title)}</h3>
    <img class="qr-side-large" src="${qrImageUrl(link)}&t=${encodeURIComponent(h.id)}" alt="QR for ${esc(h.title)}" />
