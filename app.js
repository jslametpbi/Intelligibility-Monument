...
const BUILD_VERSION = '2026.06.04-pro-360-pin';
...
function render360Model() {
  ...
  if (!stage) return;
  stage.innerHTML = `
    <button class="model-part wing wing-left" data-model-hotspot="wings" type="button" aria-label="Garuda-Inspired Wings"><span>Garuda Wings</span></button>
    <button class="model-part wing wing-right" data-model-hotspot="wings" type="button" aria-label="Garuda-Inspired Wings"><span>Garuda Wings</span></button>
    <button class="model-part spine" data-model-hotspot="spine" type="button" aria-label="Luminous Sound-Wave Spine"><span>Sound-Wave Spine</span></button>
    <button class="model-part book" data-model-hotspot="book" type="button" aria-label="Open-Book Scholarly Base"><span>Scholarly Base</span></button>
    <button class="model-part platform" data-model-hotspot="platform" type="button" aria-label="Archipelago Platform"><span>Archipelago Platform</span></button>
    <button class="model-part station station-left" data-model-hotspot="qr-audio" type="button" aria-label="Interactive QR Audio Station"><span>QR Station</span></button>
    <button class="model-part station station-right" data-model-hotspot="qr-audio" type="button" aria-label="Interactive QR Audio Station"><span>QR Station</span></button>
    <button class="model-part paving" data-model-hotspot="paving" type="button" aria-label="Intelligibility Pattern Paving"><span>Pattern Paving</span></button>
    <button class="model-part light-ring" data-model-hotspot="night" type="button" aria-label="LED Night Illumination"><span>LED Illumination</span></button>
  `;
...
function setModelRotation(value) {
  ...
  const stage = byId('modelStage');
  if (stage) stage.style.transform = `rotateX(-8deg) rotateY(${modelRotation}deg)`;
...
function toggleModelAuto() {
  ...
  modelAutoTimer = setInterval(() => setModelRotation(modelRotation + 1.2), 32);
...
function init360Events() {
  ...
  viewport.addEventListener('pointermove', e => {
    ...
    setModelRotation(modelDragStart.rotation + delta * .7);
  });
...
function renderHotspotPanel() {
  ...
  byId('hotspotPanel').innerHTML = `
    ...
    <p><strong>Related material:</strong><br><span style="color:var(--muted)">${esc(comp.name)} · ${esc(comp.material)}</span></p>
  ...
  showModal(`
    ...
    <div class="modal-grid">
      <div class="qr-card" style="margin:0">
        ...
        <img src="${qrImageUrl(target.url)}" alt="QR for ${esc(target.title)}" style="width:220px;height:220px;margin:14px auto" />
