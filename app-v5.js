const STORAGE_KEY = 'intelligibilityMonumentPro.v5.final';
const BUILD_VERSION = '2026.06.04-ultra-realistic-v5-final';
...
function setModelRotation(value) {
  ...
  if (nextIndex !== currentModelFrameIndex || !byId('modelStage')?.querySelector('.model-frame-image')) {
    ...
    if (chip) chip.textContent = `Ultra-Realistic V5 · ${currentModelFrame().label} · ${Math.round(modelRotation)}°`;
    updateModelSelection();
  }
}
...
function toggleModelAuto() {
  ...
  modelAutoTimer = setInterval(() => setModelRotation(modelRotation + 60), 700);
