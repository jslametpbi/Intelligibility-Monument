const STORAGE_KEY = 'intelligibilityMonumentPro.v4.final';
const BUILD_VERSION = '2026.06.04-ultra-realistic-v4-final';
...
function render360Model() {
  ...
  stage.innerHTML = `
    ...
    <div class="model-view-chip">Ultra-Realistic V4 · ${esc(frame.label)} · ${Math.round(modelRotation)}°</div>
...
function setModelRotation(value) {
  ...
  if (nextIndex !== currentModelFrameIndex || !byId('modelStage')?.querySelector('.model-frame-image')) {
    ...
    if (chip) chip.textContent = `Ultra-Realistic V4 · ${currentModelFrame().label} · ${Math.round(modelRotation)}°`;
    updateModelSelection();
  }
}
...
function toggleModelAuto() {
  ...
  modelAutoTimer = setInterval(() => setModelRotation(modelRotation + 6), 90);
